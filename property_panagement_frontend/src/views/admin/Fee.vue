<template>
  <div class="fee-management">
    <el-card>
      <template #header>
        <div class="card-header">
          <span>费用管理</span>
          <el-button type="primary" @click="openAddDialog">添加费用</el-button>
        </div>
      </template>
      <el-table :data="feeList" style="width: 100%">
        <el-table-column prop="id" label="ID" width="80" />
        <el-table-column prop="houseId" label="房屋ID" />
        <el-table-column prop="ownerId" label="业主ID" />
        <el-table-column prop="feeType" label="费用类型" />
        <el-table-column prop="amount" label="金额" />
        <el-table-column prop="period" label="周期" />
        <el-table-column prop="status" label="状态">
          <template #default="scope">
            <el-tag :type="scope.row.status === 1 ? 'success' : 'danger'">
              {{ scope.row.status === 1 ? '已缴费' : '未缴费' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="createTime" label="创建时间" />
        <el-table-column prop="payTime" label="缴费时间" />
        <el-table-column label="操作" width="200">
          <template #default="scope">
            <el-button type="primary" size="small" @click="openEditDialog(scope.row)">编辑</el-button>
            <el-button type="danger" size="small" @click="deleteFee(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="pagination">
        <el-pagination
          v-model:current-page="currentPage"
          v-model:page-size="pageSize"
          :page-sizes="[10, 20, 50, 100]"
          layout="total, sizes, prev, pager, next, jumper"
          :total="total"
          @size-change="handleSizeChange"
          @current-change="handleCurrentChange"
        />
      </div>
    </el-card>

    <!-- 添加/编辑费用对话框 -->
    <el-dialog v-model="dialogVisible" :title="dialogTitle">
      <el-form :model="feeForm" :rules="feeRules" ref="feeFormRef" label-width="100px">
        <el-form-item label="房屋ID" prop="houseId">
          <el-input v-model="feeForm.houseId" type="number" placeholder="请输入房屋ID"></el-input>
        </el-form-item>
        <el-form-item label="业主ID" prop="ownerId">
          <el-input v-model="feeForm.ownerId" type="number" placeholder="请输入业主ID"></el-input>
        </el-form-item>
        <el-form-item label="费用类型" prop="feeType">
          <el-input v-model="feeForm.feeType" placeholder="请输入费用类型"></el-input>
        </el-form-item>
        <el-form-item label="金额" prop="amount">
          <el-input v-model="feeForm.amount" type="number" placeholder="请输入金额"></el-input>
        </el-form-item>
        <el-form-item label="周期" prop="period">
          <el-input v-model="feeForm.period" placeholder="请输入周期"></el-input>
        </el-form-item>
        <el-form-item label="状态" prop="status">
          <el-select v-model="feeForm.status" placeholder="请选择状态">
            <el-option label="未缴费" value="0"></el-option>
            <el-option label="已缴费" value="1"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="dialogVisible = false">取消</el-button>
          <el-button type="primary" @click="submitForm">确定</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
</template>

<script setup>
import { ref, reactive, onMounted } from 'vue'
import axios from 'axios'
import { ElMessage } from 'element-plus'

const feeList = ref([])
const currentPage = ref(1)
const pageSize = ref(10)
const total = ref(0)
const dialogVisible = ref(false)
const dialogTitle = ref('添加费用')
const feeFormRef = ref(null)

const feeForm = reactive({
  id: '',
  houseId: '',
  ownerId: '',
  feeType: '',
  amount: 0,
  period: '',
  status: 0
})

const feeRules = {
  houseId: [{ required: true, message: '请输入房屋ID', trigger: 'blur' }],
  ownerId: [{ required: true, message: '请输入业主ID', trigger: 'blur' }],
  feeType: [{ required: true, message: '请输入费用类型', trigger: 'blur' }],
  amount: [{ required: true, message: '请输入金额', trigger: 'blur' }],
  period: [{ required: true, message: '请输入周期', trigger: 'blur' }]
}

onMounted(() => {
  getFeeList()
})

const getFeeList = async () => {
  try {
    const response = await axios.get('/fee/list', {
      params: {
        page: currentPage.value,
        size: pageSize.value
      }
    });
    if (response.data.code === 200) {
      feeList.value = response.data.data.list;
      total.value = response.data.data.total;
    }
  } catch (error) {
    ElMessage.error('获取费用列表失败');
  }
}

const handleSizeChange = (size) => {
  pageSize.value = size
  getFeeList()
}

const handleCurrentChange = (current) => {
  currentPage.value = current
  getFeeList()
}

const openAddDialog = () => {
  dialogTitle.value = '添加费用'
  Object.assign(feeForm, {
    id: '',
    houseId: '',
    ownerId: '',
    feeType: '',
    amount: 0,
    period: '',
    status: 0
  })
  dialogVisible.value = true
}

const openEditDialog = (row) => {
  dialogTitle.value = '编辑费用'
  Object.assign(feeForm, row)
  dialogVisible.value = true
}

const submitForm = async () => {
  await feeFormRef.value.validate()
  try {
    let response
    if (feeForm.id) {
      response = await axios.put('/fee/update', feeForm)
    } else {
      response = await axios.post('/fee/add', feeForm)
    }
    if (response.data.code === 200) {
      ElMessage.success('操作成功')
      dialogVisible.value = false
      getFeeList()
    } else {
      ElMessage.error(response.data.message)
    }
  } catch (error) {
    ElMessage.error('操作失败')
  }
}

const deleteFee = async (id) => {
  try {
    const response = await axios.delete(`/fee/delete/${id}`)
    if (response.data.code === 200) {
      ElMessage.success('删除成功')
      getFeeList()
    } else {
      ElMessage.error(response.data.message)
    }
  } catch (error) {
    ElMessage.error('删除失败')
  }
}
</script>

<style scoped>
.fee-management {
  display: flex;
  flex-direction: column;
  gap: 20px;
}

.card-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.pagination {
  margin-top: 20px;
  display: flex;
  justify-content: flex-end;
}
</style>